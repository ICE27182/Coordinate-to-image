...
def draw_c(coordinate,y_axis = True,x_limit = False):
    ...
    x_limit 默认为True 不限制
    '''
    def xline(start_value,coor):
        '''将单行的x坐标存入列表 会用2次'''
        for x in range(start_value,max(coordinate[y].keys())+1):
            if x in coordinate[y].keys() and len(str(coordinate[y][x])) == 3:
            #点  无闪烁
                coor += f'\033[{str(coordinate[y][x])[2]};{str(coordinate[y][x])[:2]}m██\033[0m'
            elif x in coordinate[y].keys() and len(str(coordinate[y][x])) == 4:
            #点  闪烁
                coor += f'\033[{str(coordinate[y][x])[2]};{str(coordinate[y][x])[:2]};{str(coordinate[y][x])[3]}m██\033[0m'
