...
#介绍
print('坐标-图 v2.1')
print('1 把颜色在coordinate中由int+str改为了全int')
print('2 发现\\033[2m 因此可以表示24色了')
print('3 x可选的可以小于0了')
...
coordinate = {   #色彩 前两位为颜色 第三位为默认/高亮/暗 第四位可选 为闪烁
    ...
    6:{0:321,1:301,2:301,3:321,4:320,5:301,6:301,7:371},
    ...
    2:{0:371,1:320,2:300,3:300,4:300,5:300,6:320,7:321},
...
for y in range(max(coordinate.keys()),min(coordinate.keys())-1,-1):
    ...
    if x_limit == False:   #允许 x < 0
        for x in range(min(coordinate[y].keys()),max(coordinate[y].keys())+1):
